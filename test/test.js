...
describe('generate wallet test', () => {
	...
	it('should generate the correct wallet with the given test vector', () => {
		...
		expect(result.accounts[0].address).to.equal('kizn_3chhhgy11k1msxtwdz4wd1i8e83fdkghzo3gpzor5mqyo5mrrjy79zpw1g34')
	})
	...
	it('should generate the correct wallet with the given test vector and a seed password', () => {
		...
		expect(result.accounts[0].address).to.not.equal('kizn_3chhhgy11k1msxtwdz4wd1i8e83fdkghzo3gpzor5mqyo5mrrjy79zpw1g34')
		...
		expect(result.accounts[0].address).to.equal('kizn_36jufjz4i91wcnnztdgab1aqh1b3fado9mynizy5a16z8payefpqo81zsshc')
	})
...
describe('import wallet with test vectors test', () => {
	...
	it('should successfully import a wallet with the official Nano test vectors mnemonic', () => {
		...
		expect(result.accounts[0].address).to.equal('kizn_1pu7p5n3ghq1i1p4rhmek41f5add1uh34xpb94nkbxe8g4a6x1p69emk8y1d')
	})
	...
	it('should successfully import a legacy hex wallet with the a seed', () => {
		...
		expect(result.accounts[0].address).to.equal('kizn_3i1aq1cchnmbn9x5rsbap8b15akfh7wj7pwskuzi7ahz8oq6cobd99d4r3b7')
	})
	...
	it('should successfully import legacy hex accounts with the a seed', () => {
		...
		expect(accounts[2].address).to.equal('kizn_1dzcca9ycmtx3q79mocmu95zdduxptp3gp5fqkmb1ownscpweggzah8cb4rb')
	})
...
describe('derive more accounts from the same seed test', () => {
	...
	it('should derive accounts from the given seed', () => {
		...
		expect(result[0].address).to.equal('kizn_1pu7p5n3ghq1i1p4rhmek41f5add1uh34xpb94nkbxe8g4a6x1p69emk8y1d')
		...
		expect(result[14].address).to.equal('kizn_3ybte61rw3w4da6ox59qqq9b6iqbr6p7rif7dqkshaoykhx64e5dbp4o1ua1')
	})
...
describe('block signing tests using official test vectors', () => {
	...
	it('should create a valid signature for a receive block', () => {
		const result = block.receive({
			...
			toAddress: 'kizn_1e5aqegc1jb7qe964u4adzmcezyo6o146zb8hm6dft8tkp79za3sxwjym5rx',
...
describe('unit conversion tests', () => {
	...
	it('should convert raw to nano', () => {
		const result = tools.convert('1000000', 'RAW', 'NANO')
